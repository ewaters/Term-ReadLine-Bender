package Term::ReadLine::Bender::ISearch;
...
sub draw { # rendering this inc mode is kinda consuming
	...
	DRAW: Term::ReadLine::Bender::draw($self, @args);
...
sub self_insert {
	...
	if ($self->{is_lock} && $chr !~ /^ctrl_/) {
		...
		goto \&Term::ReadLine::Bender::self_insert;
	}
}
...
Term::ReadLine::Bender::ISearch - a readline incremental search mode
