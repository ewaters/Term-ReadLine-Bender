package Term::ReadLine::Bender::ViCommand;

use strict;
use vars '$AUTOLOAD';
no strict 'refs';
use AutoLoader;
use base 'Term::ReadLine::Bender';
...
sub AUTOLOAD { # more intelligent inheritance
	my $sub = $AUTOLOAD;
	$sub =~ s/.*:://;
	my $m = $_[0]->can($sub) ? 'AutoLoader' : 'Term::ReadLine::Bender';
	${$m.'::AUTOLOAD'} = $AUTOLOAD;
	goto &{$m.'::AUTOLOAD'};
}
...
Term::ReadLine::Bender::ViCommand - a readline command mode

=head1 SYNOPSIS

This class is used as a mode under L<Term::ReadLine::Bender>,
...
from Term::ReadLine::Bender, key bindings are also inherited unless explicitly overloaded.
...
Switch to multiline insert mode, see L<Term::ReadLine::Bender::MultiLine>.
(private extension)
