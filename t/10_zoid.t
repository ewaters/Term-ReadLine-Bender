...
if (eval 'use Term::ReadLine; 1') {
	...
	ok $t->ReadLine eq 'Term::ReadLine::Bender', 'rl:zoid loaded';
}
else { ok 1, 'skip Term::ReadLine NOT installed, this *might* be a problem' }

my $t = Term::ReadLine::Bender->new('test');
...
print "# control-V\n";
@$t{'pos','lines'} = ([0,0], ['']);
push @Term::ReadLine::Bender::Base::_key_buffer, "\cV", "\cW";
$t->do_key();
...
print "# escape\n";
$t->press("\e");
ok ref($t) eq 'Term::ReadLine::Bender::ViCommand', 'escape to command mode';
